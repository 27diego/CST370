...
{
    return abs(abs(origin.first - dest.first) + abs(origin.second - dest.second));
}
...
vector<pair<int, int>> getNeighbors(int x, int y, int xBound, int yBound)
{
    ...
    coordinates.push_back(pair<int, int>)
...
int main()
{
    ...
    cout << AStar(pair<int, int>(3, 3), pair<int, int>(6, 6), grid)<<endl;
